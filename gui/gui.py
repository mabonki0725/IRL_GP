...
class GuiBoard:
	...
	print('in render_mdp')
		...
		pygame.draw.rect(self.screen, self.get_color(self.grid[cols][rows]),		
        ...
        pygame.draw.rect(self.screen, self.agent_color, [current_state[0] * self.pix_per_grid,
							 ...
							 (current_state[1]) * self.pix_per_grid,
        ...
        if world == 'object_world':
            ...
            for o in mdp_gw.objects:
                ...
                settings = [current_state[0] * self.pix_per_grid,(current_state[1]) * self.pix_per_grid,self.pix_per_grid, self.pix_per_grid]
		print('settings1=',current_state[0],current_state[1])
		pygame.draw.rect(self.screen, color, settings)
                ...
                settings = [current_state[0] * self.pix_per_grid + self.pix_per_grid / 4,(current_state[1]) * self.pix_per_grid + self.pix_per_grid / 4,self.pix_per_grid / 2, self.pix_per_grid / 2]
		pygame.draw.rect(self.screen, color, settings)
                self.objects[current_state].append([color, settings])
        ...
        if graphics:
            ...
            if ps in self.objects:
                ...
                pygame.draw.rect(self.screen, self.get_color(self.grid[ps[1]][ps[0]]),
				 ...
				 #[ps[0] * self.pix_per_grid, (ps[1]) * self.pix_per_grid,
                                self.pix_per_grid,
                                self.pix_per_grid])  # here we flip y-sim because the display has positive y in down directionpygame.draw.rect(self.screen, self.get_color(self.grid[ps[1]][ps[0]]),                                 
		

            pygame.draw.rect(self.screen, self.agent_color,
                             [s[0] * self.pix_per_grid, (len(self.grid) - 1 - s[1]) * self.pix_per_grid,
			     #[s[0] * self.pix_per_grid, (s[1]) * self.pix_per_grid,			     
                              self.pix_per_grid,
